...
class SaleWizard(models.TransientModel):
    ...
    session_id = fields.Many2one('academy.session', required=True, default=_default_session)

    session_student_ids = fields.Many2many('res.partner', string='Students in current session', related='session_id.student_ids', help='These are the students currently in the session')

    student_ids = fields.Many2many('res.partner', string='Students for sales order')
